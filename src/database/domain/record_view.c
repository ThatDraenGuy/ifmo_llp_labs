...
result_t record_view_next(struct record_view *self, struct record **result) {
  ...
  *result = single_record_holder_get_values(self->current_record_holder);

  while (item_iterator_has_next(self->item_it)) {
    single_record_holder_clear(self->next_record_holder);

    item_t item = ITEM_NULL;
    TRY(item_iterator_next(self->item_it, &item));
    CATCH(error, THROW(error))

    TRY(record_deserialize(
        item, single_record_holder_get_values(self->next_record_holder)));
    CATCH(error, THROW(error))

    bool predicate_res = false;
    TRY(predicate_apply(
        self->predicate,
        single_record_holder_get_values(self->next_record_holder),
        &predicate_res));
    CATCH(error, THROW(error))
